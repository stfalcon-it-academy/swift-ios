...
class ReminderInfoVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if self.index != nil {
            let rightBarButton = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.Save, target: self, action: #selector(ReminderInfoVC.saveReminder))
            self.navigationItem.rightBarButtonItem = rightBarButton
        } else {
            let rightBarButton = UIBarButtonItem(barButtonSystemItem: UIBarButtonSystemItem.Add, target: self, action: #selector(ReminderInfoVC.addReminder))
            self.navigationItem.rightBarButtonItem = rightBarButton
    ...
    func updateReminderModel() {
        ...
        if self.datePicker.date.compare(NSDate()) == NSComparisonResult.OrderedAscending {
            ...
            UIAlertView(title: "Error", message: "Cannot set reminder for an earlier time", delegate: nil, cancelButtonTitle: "OK").show()
