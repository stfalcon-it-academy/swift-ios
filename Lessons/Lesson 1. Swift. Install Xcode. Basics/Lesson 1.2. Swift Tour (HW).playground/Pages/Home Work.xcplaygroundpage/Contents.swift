/*:
 ...
 3. Используйте `\()`, чтобы добавить выражение вычисления числа с плавающей точкой в строку.
    Также, попробуйте вставить чье-нибудь имя в приветствие.
 ...
 11. Создайте еще один подкласс `NamedShape` под названием `Circle`. Он должен принимать значения `radius` и `name` в качестве входных аргументов для совей инициализации. Для класса `Circle` реализуйте методы `area()` и `simpleDescription()`.
 ...
 */
//: [Generics](@previous) | [Home Work](@next)
